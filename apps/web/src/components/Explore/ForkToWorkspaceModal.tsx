...
export interface ForkToWorkspaceModalProps {
    ...
    onFork: (params: { documentId: string; workspaceId: string }) => Promise<void>;
...
function WorkspaceRow({ workspace, selected, onSelect, disabled }: WorkspaceRowProps) {
    return (
        <button
            type="button"
            disabled={disabled}
        ...
        >
            ...
            <span
                className={[
                    ...
                    selected ? "bg-violet-500 opacity-100 scale-100" : "opacity-0 scale-75",
                ].join(" ")}
            >
...
export function ForkToWorkspaceModal({
    ...
    async function handleFork() {
        ...
        try {
            ...
            setStatus("error");
            setError(err instanceof Error ? err.message : "Failed to fork. Please try again.");
        }
    }
    ...
    const buttonClass = useMemo(() => {
        ...
        if (status === "loading")
            return `${base} bg-[#A308F0] cursor-not-allowed`;
        if (!canFork)
            return `${base} bg-[#A308F0] cursor-not-allowed`;
