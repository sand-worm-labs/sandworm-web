...
function Table(props: Props) {
  ...
  return (
    ...
    >
      ...
      >
        <thead className="bg-[#F9FAFB] dark:bg-base-800 sticky top-0">
          <tr className="divide-x h-10 divide-hover-border dark:divide-border-dark">
            {props.columns.map(column => {
              ...
              return (
                <th
                  scope="col"
                  className={clsx(
                    "p-2 text-ink-400  whitespace-nowrap font-bold border-b dark:border-border-dark hover:bg-gray-100 dark:hover:bg-base-800 cursor-pointer",
        ...
        <tbody className="bg-white dark:bg-header-surface overflow-y-scroll">
          {props.rows.map((row, rowIndex) => (
            <tr
              key={rowIndex}
              className="divide-x divide-hover-border dark:divide-border-dark "
            >
                  ...
                  <td
                    ...
                    className={clsx(
                      rowIndex === props.rows.length - 1 && !props.isDashboard
                        ? "border-b-0"
                        : "border-b",
                      "px-3 py-2 dark:text-ink-800 text-ink-500 whitespace-nowrap border-hover-border dark:border-border-dark font-medium "
                    )}
                  >
                    {cell === null ? (
