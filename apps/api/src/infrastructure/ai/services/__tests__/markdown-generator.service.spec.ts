...
function makeService() {
  ...
  const service = new MarkdownGeneratorService(configService, httpService, workspaceService);
...
describe('MarkdownGeneratorService', () => {
  describe('edit', () => {
    it('posts to /markdown/edit with workspace model and api key, returning the response data', async () => {
      ...
      httpService.post.mockReturnValue(of({ data: { content: '# Title' } }));

      const result = await service.edit(CONTEXT, 'add a heading');

      expect(httpService.post).toHaveBeenCalledWith(
        'http://ai.local/markdown/edit',
        {
    ...
    it('propagates an error when the HTTP call fails', async () => {
      ...
      await expect(service.edit(CONTEXT, 'add a heading')).rejects.toThrow('network down');
    });
  });
});
