...
export interface CreateVisualizationTask {
...
export class VisualizationService {
    ...
    private generateVisualizationCode(config: VisualizationConfig): string {
        const { dataframe, input } = config

        const filters = input.filters.filter((f) => !isUnfinishedVisualizationFilter(f))
...
if "${dataframe.name}" in globals():
    ...
    private processOutputs(
                ...
                case 'stdio':
                    ...
                    if (output.name === 'stdout') {
                        ...
                        if (parsed) {
                            ...
                            outputParsingErrors.push(
                                new Error(`Failed to parse output: ${output.text}`)
                            )
                        }
                    }
                    break
        ...
        if ((pythonErrors.length > 0 || outputParsingErrors.length > 0) && !result) {
