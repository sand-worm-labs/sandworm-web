...
export async function makeQuery(
    ...
    let logger = new Logger('makeQuery');
    ...
    const resultPromise = queryPromise.then(async (): Promise<RunQueryResult> => {
        ...
        const code = `
def _sandworm_read_query():
    import pandas as pd
    retries = 3
    while retries > 0:
        try:
            return pd.read_parquet("/home/jupyteruser/.sandworm/query-${queryId}.parquet.gzip")
        except:
            retries -= 1
            if retries == 0:
                raise
...
${dataframeName} = _sandworm_read_query()
...
if "${dataframeName}" in globals():
    ...
    result = {
      ...
      "dashboardPageCount": int(len(${dataframeName}) / ${pageOptions.dashboardPageSize
        } + 1),
...
export async function listDataFrames(
    ...
    let logger = new Logger('listDataFrames');
    const code = `
def _sandworm_list_dataframes():
    import pandas as pd
    import json
    dataframes = []
...
_sandworm_list_dataframes()
del _sandworm_list_dataframes`
